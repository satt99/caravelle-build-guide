...
1. ダイオード×2を取り付ける
    ...
    | ---- | ---- | ---- |
    | 片側のパッドに予備はんだを盛ります | ダイオードの向きに注意しつつ、予備はんだをつけたパッドにダイオードをはんだ付けします（基板の逆`コ`の字型のシルクの縦線とダイオードの印字の縦線の向きを揃えます） | もう片側のパッドもはんだ付けします |
...
1. PHコネクタ付きケーブルを基板の左側のスルーホールに取り付ける
    ...
    | ---- | ---- |
    | 赤いケーブルをVCCに、黒いケーブルをGNDにはんだ付けします | ケーブルがショートしないように必要に応じて余分な配線を切ります |
...
1. 2個目の電池用基板も1個目と同様の手順で部品を取り付けます。
    ...
    | ---- |
    | 2個めの電池基板ではケーブルを1個目と逆の位置に取り付けます |
...
1. スレーブ側（右手側）を起動する
    ...
    * スレーブ側が起動しない場合は、電池基板と本体基板のはんだづけが正しく行われていることを再度確認します
...
1. マスタ側（左手側）を起動する
    ...
    * マスタ側が起動しない場合は、電池基板と本体基板のはんだづけが正しく行われていることを再度確認します
...
1. 入力テストを行う
    ...
    * デフォルトのファームウェアではキーボード側がUS配列である点や、スマートフォンなどでは正しく入力されないキーコードもある点に注意
